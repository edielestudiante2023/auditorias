...
<head>
    ...
    <title><?= esc($title ?? 'Editar Cliente') ?> | Auditorías</title>
...
<div class="container py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h4 class="mb-0"><i class="bi bi-building"></i> <?= esc($title ?? 'Editar Cliente') ?></h4>
        ...
        <div class="card-body">
            <form action="<?= esc($form_action) ?>" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control <?= isset($errors['razon_social']) ? 'is-invalid' : '' ?>" id="razon_social" name="razon_social" value="<?= esc(old('razon_social', $cliente['razon_social'] ?? '')) ?>" required minlength="3">
                    <?php if (!empty($errors['razon_social'])): ?><div class="invalid-feedback"><?= esc($errors['razon_social']) ?></div><?php endif; ?>
                </div>

                <div class="mb-3">
                    <label for="nit" class="form-label">NIT</label>
                    <input type="text" class="form-control <?= isset($errors['nit']) ? 'is-invalid' : '' ?>" id="nit" name="nit" value="<?= esc(old('nit', $cliente['nit'] ?? '')) ?>" required>
                ...
                <div class="form-check form-switch mb-3">
                    <?php $activo = old('estado', $cliente['estado'] ?? 'activo') === 'activo'; ?>
